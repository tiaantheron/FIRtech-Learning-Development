...
export function WorkbookEditor({ route }: { route: string }) {
  ...
  const [pendingRemoval, setPendingRemoval] = useState<EditableRow | null>(null)
  ...
  return <section className="mb-6 rounded-lg border bg-card p-4 print:hidden">
    ...
    {open && <div className="mt-4 space-y-4">
      ...
      {pendingRemoval && <div role="alertdialog" aria-label="Confirm record action" className="flex flex-wrap items-center gap-3 rounded border border-warning bg-muted p-4"><p>{lifecycleField ? "Archive" : "Remove"} {String(pendingRemoval.values[view.headers[0]])}? Undo remains available during this session.</p><Button disabled={loading} onClick={() => void submit("delete", pendingRemoval)}>Confirm {lifecycleField ? "archive" : "remove"}</Button><Button variant="outline" disabled={loading} onClick={() => setPendingRemoval(null)}>Keep record</Button></div>}
