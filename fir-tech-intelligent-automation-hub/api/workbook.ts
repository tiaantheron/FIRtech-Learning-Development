...
export default async function handler(request: Request): Promise<Response> {
  try {
    ...
    if (request.method === "GET") {
      ...
      // The client validates the complete dashboard workbook schema after loading.
      // Keeping this hot read path free of ExcelJS avoids a serverless cold-start timeout.
      const current = await downloadWorkbook(config, revision)
